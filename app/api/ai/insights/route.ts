...
export async function POST(): Promise<NextResponse> {
  try {
    ...
    if (posts.length === 0) {
      return NextResponse.json({ error: "No posts to analyze yet" }, { status: 400 });
    }
    ...
    for (const pattern of patterns) {
      await query(
        ...
        [pattern.kind, pattern.title, pattern.body, pattern.headline_metric, process.env.ANTHROPIC_MODEL ?? "claude-sonnet-5"],
      );
    }
